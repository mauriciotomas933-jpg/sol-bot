...
def place_sell(symbol, qty):
    """Vende tokens — usa baseCoin con qty calculado desde la compra"""
    qty_rounded = round(float(qty), 4)
    if qty_rounded <= 0:
        ...
        if not in_trade:
            ...
            try:
                ...
                if change_pct >= TAKE_PROFIT_PCT or trailing_triggered:
                    try:
                        qty_to_sell = trade_info.get("qty", USDT_PER_TRADE / buy_price)
                        place_sell(trade_symbol, qty_to_sell)
