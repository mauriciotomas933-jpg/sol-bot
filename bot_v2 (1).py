...
MIN_SCORE       = 65    # más exigente — solo señales muy fuertes
...
def place_order(symbol, side, qty, usdt_amount=None):
    ...
    if side == "Buy" and usdt_amount:
        ...
        body = {"category": "spot", "symbol": symbol, "side": side,
                "orderType": "Market", "qty": str(round(qty, 4)), "marketUnit": "baseCoin"}
    ...
    while True:
        ...
        if not in_trade:
            ...
            if best_score >= MIN_SCORE and best_symbol:
                ...
                try:
                    place_order(best_symbol, "Buy", 0, usdt_amount=USDT_PER_TRADE)
                    in_trade     = True
            ...
            try:
                ...
                if change_pct >= TAKE_PROFIT_PCT:
                    try:
                        ...
                        place_order(trade_symbol, "Sell", bal)
                        ganancia = bal * (price - buy_price)
                        send_telegram(
                            f"💰 <b>VENTA — GANANCIA {trade_coin}</b>\n\n"
