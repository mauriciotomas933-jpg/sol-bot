...
def place_sell(symbol, qty, coin=""):
    ...
    result = r.json()
    if result["retCode"] != 0:
        # Si falla por balance, intentar con 98%
        qty_retry = round(float(qty) * 0.98, decimals)
        body["qty"] = fmt.format(qty_retry)
        body_str = json.dumps(body, separators=(',', ':'))
        r = requests.post(f"{BASE_URL}/v5/order/create", headers=bybit_headers(body_str), data=body_str, timeout=10)
        r.raise_for_status()
        result = r.json()
            ...
            try:
                ...
                if change_pct >= TAKE_PROFIT_PCT or trailing_triggered:
                    try:
                        ...
                        qty_to_sell = bal if bal > 0 else trade_info.get("qty", USDT_PER_TRADE / buy_price)
